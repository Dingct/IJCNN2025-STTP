...
parser.add_argument("--input_dim", type=int, default=7, help="input_dim")
...
class trainer:
    def __init__(
        ...
        self.model = STTP(
            device, input_dim, channels, num_nodes, input_len, output_len, num_regions,dropout
        )
    ...
    def train(self, input, real_val):
        ...
        real = torch.unsqueeze(real_val[...,0], dim=-1)  # 64 1 170 12
        predict = self.scaler.inverse_transform(output)  # 64 1 170 12
        real = self.scaler.inverse_transform(real)  # 64 1 170 12
...
def main():
    ...
    if args.data == "PEMS08": # 17
        args.num_nodes = 170

    elif args.data == "CAD3": # 96
        args.num_nodes = 480
        num_regions = 96

    elif args.data == "CHI_TAXI": # 8
        args.num_nodes = 77
        num_regions = 8

    elif args.data == "PEMS07M": # 23
        args.num_nodes = 228
        num_regions = 23
    
    elif args.data == "bike_drop":  # 25
        args.num_nodes = 250
        num_regions = 25
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y) in enumerate(train_loader):
            trainx = x.to(device)  # 64 12 170 1
            trainy = y.to(device)
        ...
        if mvalid_loss < loss:
            ...
            if i <= 50:
                ...
                for iter, (x, y) in enumerate(test_loader):
                    ...
                    test_labels.append(y[...,0].squeeze())
